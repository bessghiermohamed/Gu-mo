...
// round 51 — طلب المالك: «المسجَّل دخوله لا يرى AdSense في كل مرة،
// بل يدخل تطبيقه مباشرة»:
// a visitor carrying a session cookie is an EXISTING user — they go
// STRAIGHT to the app instead of being shown the pre-login marketing
// pages (which is where all AdSense units live) on every visit.
// Cookie-presence only (no DB hit): /app enforces the real session, so
// a stale cookie simply lands on the login screen — no loop.
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <main id="main">
        {/* ===================== HERO =====================
            round 51 («Re-banner» بطلب المالك): لافتة الهيرو تعود —
            بنسختها الخضراء الأكاديمية المستعادة، بعمودين على الشاشات
            الكبيرة ومتكدسة في الوسط على الهاتف. */}
