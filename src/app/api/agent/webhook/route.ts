// Murad's Telegram door: messages (chat/commands/goals) + approval buttons.
import { NextResponse } from 'next/server';
import { WEBHOOK_SECRET } from '@/lib/community/config';
...
export async function GET() {
  return NextResponse.json({ ok: true, service: 'murad-agent', version: 1, time: new Date().toISOString() });
}
