...
export async function DELETE(req: NextRequest) {
  ...
  try {
    if (isVercel) {
      const supabase = await createSupabaseServerClient();
      const { data: spec } = await supabase.from("specialties").select("id, name_ar").eq("id", specId).maybeSingle();
      if (!spec) return NextResponse.json({ error: "التخصص غير موجود" }, { status: 404 });
      // guard: block while anything still depends on the specialty.
      ...
      const spec = await db.specialty.findUnique({ where: { id: specId }, select: { nameAr: true } });
      if (!spec) return NextResponse.json({ error: "التخصص غير موجود" }, { status: 404 });
    ...
    return NextResponse.json({ ok: true, message: "تم حذف التخصص" });
  } catch (e) {
    return NextResponse.json({ error: `خطأ: ${(e as Error).message}` }, { status: 500 });
  }
}
